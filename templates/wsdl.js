...
var neededArgs, NS, TableX, ReqName, ResName;
...
'\n        <import namespace="http://xsd.planetcross.net/planetcross.xsd" schemaLocation="http://xsd.planetcross.net/planetcross.xsd"/>' +
...
// "\n             <!-- " + JSON.stringify(result) + "  -->" +
 result.cols.map((colName, i) =>
    " <xs:element name='" + escXml(colName) + "' type='xs:" + xmlType(result.types[i], result.rawTypes[i]) + "'>" +
    "\n   <xs:annotation><xs:appinfo>" +
...
'\n    <wsdl:part name="targetUserId"    element="px:targetUserId"/>' +
...
'\n            <soap:header use="literal" message="tns:requestHeader" part="targetUserId"/>' +
...
'\n            <soap:address location="' + escXml(NS.replace(/\.wsdl$/, '.soap'))+ '"/>' +
'\n        </wsdl:port>' +
'\n    </wsdl:service>'  +
'\n    </wsdl:definitions>' +
'\n';
...
// using Set to make update list unique because variables and fields can have same name
if (vars.servtype === 'update') neededArgs = [...new Set(getVars(result.explainQuery().where).concat(getFields(result.explainQuery(true).select)))];
if (vars.servtype === 'delete') neededArgs = getVars(result.explainQuery().where);
if (vars.servtype === 'insert') neededArgs = getFields(result.explainQuery(true).select);
