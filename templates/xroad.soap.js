...
toString = x => (typeof b !== 'undefined' && b !== null && b.toString) ? b.toString() : b; // this is because of buffer actually
rmXmlRestricted = x => typeof x !== 'string' ? x : x.replace(/[\x00-\x08\x0B-\x0C\x0E-\x1F\x7F-\x84\x86-\x9F]/g, '');
escXml =          x => typeof x !== 'string' ? x : x.replace(/&/g, '&amp;').replace(/[<>'"]/g, c => escXml.chars[c]);
escXml.chars = {
    '<': '&lt'
  , '>': '&gt'
  , "'": '&apos;'
  , '"': '&quot;'}
xmlStr = x => rmRestricted(escXml(toString(x)));
// still note that for trusted strings only escXml is used (as table/field names etc)

normalizeName = x => typeof x === 'string' ? x.replace(/[^a-z0-9]+/ig, '_') : x;

var T = { str: 'string' };
var cols = JSON.parse(JSON.stringify(result.cols));

var tableName = req.tableAs || req.table;
var S = normalizeName(tableName) + 'Response';
...
if (! ['insert','update','delete'].includes(vars.servtype)) {
  O += result.data.map(r => (
    '<row>\n' +
      cols.map((c,ci) => '<' + c + ' type="' + ((x => T[x] || x)(result.types[ci])) + '"' + (r[c] === null ? '/>' : ('>' + xmlStr(r[c]) + '</' + c + '>'))).join('\n') +
    '\n</row>'
  )).join("\n")
} else {
